...
<section id="page-hero-section" class="section">
			...
			<ol class="breadcrumb-modern__list">
				{#each breadcrumbs as crumb, index}
					<li class="breadcrumb-modern__item">
						...
						{/if}
						{#if index < breadcrumbs.length - 1}<span class="breadcrumb-modern__divider" aria-hidden="true"></span>{/if}
					</li>
				{/each}
			</ol>
		</nav>
	...
	{#if pageData.headerBackgroundPublicId}
		<div
			...
			role="img"
			aria-label="Silk Road noodles background"
...
<div class="timeline-container">
	<div style="max-width: 1400px; margin: 0 auto; padding: 2rem min(2rem, 4vw); position: relative;">
		<section class="dishes-intro"><p>Lagman in Kazakhstan blends Silk Road heritage with local precision and is one of the most rewarding bowls for culinary travelers.</p></section>
				...
				<div class="dishes-list">
					<article class="dish-article">
						...
						<div class="dish-article-content">
							<div class="dish-header"><span class="dish-tier tier-1">Silk Road Noodles</span><h3>{section.title}</h3></div>
		...
		{/each}
		<section class="themed-content-block" style="margin-top: 3rem;"><Comments postId="silkRoadNoodles" /></section>
	</div>
</div>

{#if windowWidth <= 1023}<nav class="mobile-bottom-nav-local" aria-label="Mobile navigation"><a href="/" aria-label="Home"><i class="fa fa-home" aria-hidden="true"></i></a></nav>{/if}
