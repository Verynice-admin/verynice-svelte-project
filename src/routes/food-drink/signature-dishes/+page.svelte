<script>
	...
	const defaultPage = {
		seo: {
			title: 'Signature Dishes | VeryNice',
			description: 'Explore the iconic dishes of Kazakhstan.'
		},
		mainTitle: 'Signature Dishes of Kazakhstan',
		headerDescription: 'Discover the flavors that define Kazakh cuisine.',
...
<div class="timeline-container">
	<div style="max-width: 1400px; margin: 0 auto; padding: 2rem min(2rem, 4vw); position: relative;">
		...
		<section class="dishes-intro">
			<p>
				Kazakh cuisine is deeply rooted in nomadic traditions, emphasizing <strong
					>hearty, meat-based dishes</strong
				>
				designed to sustain travelers across the vast steppe. Every meal is a celebration of hospitality,
				meant to be shared around the <em>dastarkhan</em> (family table).
			</p>
		</section>
		...
		<nav class="category-nav" aria-label="Jump to category">
			{#each categories as cat (cat.id)}
				<a href="#{cat.id}" class="category-nav-link">
					<span class="nav-icon">{categoryIcons[cat.id] || '🍽️'}</span>
		...
		<section class="themed-content-block" style="margin-top: 3rem;">
			<Comments postId="signatureDishes" />
		</section>
	</div>
</div>
