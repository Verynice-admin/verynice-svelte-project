<script>
	...
	const defaultPage = {
		seo: { title: 'Mythology, Folklore & Beliefs | Culture | VeryNice', description: 'Explore Kazakh epic heroes, the Tulpar, and ancient shamanic traditions.' },
	...
	const tocGroups = [
		{ id: 'core-beliefs', title: 'Core Beliefs', children: ['tengrism', 'umai', 'zher-su', 'creation'] },
		{ id: 'cosmos', title: 'Cosmos & Spirit World', children: ['three-worlds', 'shamanism'] },
		{ id: 'sacred-animals', title: 'Sacred Animals', children: ['totemic', 'nature-animism'] },
		{ id: 'heroes-legends', title: 'Heroes & Legends', children: ['samruk', 'epic-heroes', 'magic-horses', 'korkyt'] },
		{ id: 'evil-spirits', title: 'Evil Spirits & Monsters', children: ['albasti', 'zhalmauyz', 'azhdarha', 'peri-jinn'] },
		{ id: 'traditions', title: 'Traditions & Wisdom', children: ['protective', 'ancestor', 'folk-medicine', 'folktales'] }
	];

	let pageData = { ...defaultPage, ...(page ?? {}) };
	$: breadcrumbs = pageData.breadcrumbs || defaultPage.breadcrumbs;
	$: tocSections = tocGroups.map(group => ({
		id: group.id,
		title: group.title,
...
<section id="page-hero-section" class="section">
	...
	{#if pageData.headerBackgroundPublicId}
		<div class="header-background" role="img" aria-label="Kazakh mythology background" style={`--hero-bg-url: url("${getCloudinaryUrl(pageData.headerBackgroundPublicId, { width: 2200, height: 1600, crop: 'fill', gravity: 'north', quality: 'auto:good', fetch_format: 'auto' })}")`}>
...
<div class="timeline-container">
	<div style="max-width: 1260px; margin: 0 auto; padding: 2rem min(2rem, 4vw); position: relative;">
		<section class="intro">
			<p>Kazakh spiritual life blends <strong>ancient Turkic shamanism, Islam, and folk beliefs</strong>, creating a rich tapestry of stories, spirits, and cosmic understanding.</p>
		</section>
		...
		{#if sections && sections.length}
				...
				{#each group.children as childId}
					{@const section = sections.find(s => s.id === childId)}
					{#if section}
						<section class="category-section" id={section.id}>
							<header class="category-header">
								<span class="category-icon">{sectionCodes[section.id] || 'Myth'}</span>
		...
		<section class="themed-content-block">
			<Comments postId="cultureMythologyFolklore" />
		</section>
	</div>
</div>
...
<style>
	...
	.group-header { margin: 3rem 0 1.5rem; padding: 1rem 1.5rem; background: linear-gradient(135deg, rgba(5, 115, 179, 0.1), rgba(3, 80, 130, 0.15)); border-left: 4px solid rgba(5, 115, 179, 0.8); border-radius: 0 8px 8px 0; }
	.group-title { font-size: 1.5rem; color: #000000; margin: 0; }
