<script>
	...
	const defaultPage = {
		seo: {
			...
			description: 'Explore Kazakh musical traditions, dombra melodies, and traditional dances.'
		},
...
<section id="page-hero-section" class="section">
	...
	{#if pageData.headerBackgroundPublicId}
		<div class="header-background" role="img" aria-label="Kazakh music and dance background" style={`--hero-bg-url: url("${getCloudinaryUrl(pageData.headerBackgroundPublicId, { width: 2200, height: 1600, crop: 'fill', gravity: 'north', quality: 'auto:good', fetch_format: 'auto' })}")`}>
...
<div class="timeline-container">
	<div style="max-width: 1260px; margin: 0 auto; padding: 2rem min(2rem, 4vw); position: relative;">
		<section class="intro">
			<p>Kazakh musical traditions <strong>echo the sounds of the steppe</strong> — the wind through grass, galloping horses, and the human voice expressing love, longing, and heroism.</p>
		</section>
			...
			{#each sections as section (section.id)}
				<section class="category-section" id={section.id}>
					<header class="category-header">
						<span class="category-icon">{sectionCodes[section.id] || 'Music'}</span>
		...
		<section class="themed-content-block">
			<Comments postId="cultureMusicDance" />
		</section>
	</div>
</div>
