...
<svelte:head>
	<title>Business Sign In - VERYNICE.kz</title>
	<meta name="description" content="Sign in or register as a business to manage your listings" />
</svelte:head>
	...
	<div class="auth-container">
		...
		<div class="auth-card">
			<div class="auth-header">
				...
				<h1>{isSignUp ? 'Register Business' : 'Business Sign In'}</h1>
				<p class="auth-subtitle">
					{isSignUp 
						? 'Create a business account to manage your listings' 
			...
			<form on:submit|preventDefault={handleSubmit}>
				{#if isSignUp}
					<div class="form-group">
						<label for="name">Business Name / Full Name</label>
						<input 
							type="text" 
							id="name" 
							bind:value={name} 
				...
				<button type="submit" class="submit-button business" disabled={isLoading}>
					{#if isLoading}
...
<style>
	...
	.form-group input {
		width: 100%;
		padding: var(--vnk-spacing-sm, 0.75rem);
		border: 2px solid rgba(200, 154, 46, 0.3);
		border-radius: var(--vnk-border-radius-md, 0.75rem);
		font-size: 1rem;
	...
	.submit-button.business {
		background: #c89a2e;
	}

	.submit-button.business:hover:not(:disabled) {
		background: #a88025;
