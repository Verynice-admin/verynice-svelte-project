...
const securityHandle: Handle = async ({ event, resolve }) => {
	...
	// Reporting API v1: declare the endpoint group referenced by the CSP report-to directive.
	response.headers.set('Reporting-Endpoints', 'csp-endpoint="/api/csp-report"');

	// Remove X-DNS-Prefetch-Control — the browser default for HTTPS is already 'on',
	// so the header is redundant and unnecessarily leaks visited hostnames on monitored networks.
	...
	if (event.url.pathname.startsWith('/_app/') || event.url.pathname.startsWith('/vendor/')) {
		...
		// Auth endpoints, admin pages, and authenticated user dashboards must never be
		// cached by any CDN layer — they contain session-specific data.
