...
export async function createAltynEmelPage() {
    ...
    logger.info('[seed] Starting Altyn-Emel content enhancement');
    ...
    try {
        ...
        await batch.commit();
        logger.info('[seed] Altyn-Emel seeded successfully');

    } catch (e) {
        logger.error('[seed] Error uploading Altyn-Emel data', { err: String(e) });
    }
}
