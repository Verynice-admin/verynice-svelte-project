...
// Page metadata
const pageData = {
  id: 'signatureDishesPage',
  mainTitle: 'Signature Dishes & Traditional Drinks of Kazakhstan',
  headerDescription: 'From the national dish beshbarmak to refreshing shubat, discover the flavors and beverages that define Kazakh cuisine.',
  ...
  seo: {
    title: 'Signature Dishes | Food & Drinks | VeryNice',
    description: 'Explore the iconic dishes of Kazakhstan - from beshbarmak to baursak.'
  },
  updatedAt: now,
  createdAt: now
};
...
const dishes = [
  // === MAIN MEAT DISHES ===
  {
    ...
    kazakh: 'Бешбармақ / Ет',
    category: 'main-meat',
    description: 'The national dish: boiled meat and noodles served family-style, traditionally called et.',
    longDescription: 'Et is the original name for Kazakhstan’s national dish, widely known today as beshbarmak. It centers on tender boiled meat (often horse or lamb) served over hand-cut noodles with a rich broth called sorpa. Eaten by hand and shared at gatherings, it symbolizes unity and hospitality.',
...
async function seed() {
  ...
  // Update page metadata for signature dishes section
  console.log('[seed] Updating restaurantsPage with signature dishes metadata...');
  await pageRef.set({
    signatureDishesTitle: pageData.mainTitle,
    signatureDishesDescription: pageData.headerDescription,
    signatureDishesHeroKicker: pageData.heroKicker,
    signatureDishesHeroPublicId: pageData.headerBackgroundPublicId,
    updatedAt: now
  }, { merge: true });

  // Write categories to restaurantsPage/signatureDishesCategories/
  console.log('[seed] Writing categories to restaurantsPage/signatureDishesCategories/...');
  const categoriesBatch = db.batch();
  for (const cat of categories) {
    const catRef = pageRef.collection('signatureDishesCategories').doc(cat.id);
    categoriesBatch.set(catRef, { ...cat, updatedAt: now }, { merge: true });
  }
  await categoriesBatch.commit();

  // Write dishes to restaurantsPage/signatureDishes/ (existing location)
  console.log('[seed] Writing dishes to restaurantsPage/signatureDishes/...');
  const dishesBatch = db.batch();
  for (const dish of dishes) {
    const dishRef = pageRef.collection('signatureDishes').doc(dish.id);
    dishesBatch.set(dishRef, { ...dish, updatedAt: now }, { merge: true });
  }
  await dishesBatch.commit();

  // Delete old/removed dishes that should no longer exist
  const dishesToDelete = ['et', 'borsha', 'mi-sorpa', 'karyn-sorpa', 'tandyryp-sorpa', 'mi'];
  console.log('[seed] Deleting removed dishes:', dishesToDelete.join(', '));
  const deleteDishBatch = db.batch();
  for (const dishId of dishesToDelete) {
    const dishRef = pageRef.collection('signatureDishes').doc(dishId);
    deleteDishBatch.delete(dishRef);
  }
  await deleteDishBatch.commit();
  console.log('[seed] Deleted old dishes');

  // Clean up: delete the old signatureDishesPage if it exists
  const oldPageRef = db.collection('pages').doc('signatureDishesPage');
  const oldPageSnap = await oldPageRef.get();
  if (oldPageSnap.exists) {
    console.log('[seed] Cleaning up old signatureDishesPage...');
    // Delete subcollections first
    const subcollections = ['categories', 'dishes'];
    for (const subCol of subcollections) {
      const subColSnap = await oldPageRef.collection(subCol).get();
      if (!subColSnap.empty) {
        const deleteBatch = db.batch();
        subColSnap.docs.forEach(doc => deleteBatch.delete(doc.ref));
        await deleteBatch.commit();
      }
    }
    await oldPageRef.delete();
    console.log('[seed] Deleted old signatureDishesPage document');
  }

  console.log(`[seed] ✅ Signature Dishes seeded successfully!`);
  console.log(`[seed]    - Location: pages/restaurantsPage/signatureDishes/`);
  console.log(`[seed]    - Categories: ${categories.length} (in signatureDishesCategories/)`);
  console.log(`[seed]    - Dishes: ${dishes.length}`);
}
