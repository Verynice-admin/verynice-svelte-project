...
async function renovateRakhmanov() {
    const db = await initFirebase();
    console.log('🚀 Starting Rakhmanov Springs Renovation...');
    ...
    for (const regionDoc of regionsSnap.docs) {
        ...
        const snap = await attractionsRef.where('title', '>=', 'Rakhmanov').where('title', '<=', 'Rakhmanov\uf8ff').get();
        ...
        if (!attractionDoc) {
            ...
            const found = all.docs.find(d => {
                ...
                return (t.includes('rakhmanov') || t.includes('springs'));
            });
            if (found) {
                attractionDoc = found;
                regionTitle = regionDoc.data().title;
    ...
    if (!attractionDoc) {
        console.error('❌ Rakhmanov attraction not found!');
        return;
    }
...
async function updateAttraction(doc, region) {
    console.log(`✅ Found Springs in [${region}]: ${doc.data().title} (${doc.id})`);
...
renovateRakhmanov().catch(console.error);
